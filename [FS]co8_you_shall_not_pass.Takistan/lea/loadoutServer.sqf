...
if (!isNil "Onkel_Ali") then {
removeallweapons Onkel_Ali;
removeallassigneditems Onkel_Ali;
removeHeadgear Onkel_Ali;
removeUniform Onkel_Ali;
removeBackpack Onkel_Ali;
removeVest Onkel_Ali;
removeGoggles Onkel_Ali;

if (count weapons Onkel_Ali > 0) then {
  private['_type', '_muzzles'];
  _type = ((weapons Onkel_Ali) select 0);
  _muzzles = getArray(configFile >>"cfgWeapons">> _type >>"muzzles");
  if (count _muzzles > 1) then {
     Onkel_Ali selectWeapon (_muzzles select 0);
  }
  else {
     Onkel_Ali selectWeapon _type;
  };
};
};
